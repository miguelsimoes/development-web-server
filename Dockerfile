...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y -qq libssl1.0.0 php7.2-fpm php7.2-apcu-bc php7.2-apcu php7.2-bcmath php7.2-curl php7.2-cli php7.2-json php7.2-mbstring php7.2-memcached php7.2-mysql php7.2-redis php7.2-soap php7.2-sqlite3 php7.2-xdebug php7.2-xmlrpc php7.2-xsl wget
...
RUN sed -i -e "s/;opcache.enable_cli=0/opcache.enable_cli=0/g"                                            /etc/php/7.2/cli/php.ini
...
RUN sed -i -e 's/session.save_handler\s*=\s*files/session.save_handler=memcached/g'		          /etc/php/7.2/fpm/php.ini
RUN sed -i -e 's#;session.save_path\s*=\s*"/var/lib/php/sessions"#session.save_path=172.17.2.1:11211#g'   /etc/php/7.2/fpm/php.ini
